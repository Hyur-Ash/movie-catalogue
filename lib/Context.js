...
export const ContextProvider = ({children}) => {
    ...
    const [version, setVersion] = useLocalStorage("versionCounter", currentVersion);
    useEffect(()=>{
    ...
    const getMedia = async (mediaType, id, lang) => {
        ...
        try{
            ...
            if(media[lang].belongs_to_collection){
                const collectionInfo = await axios.get(`${tmdb_main_url}/collection/${media.en.belongs_to_collection.id}`, {params: {...params, language: lang}});
                media[lang].collection = collectionInfo.data;
            }
            if(media.en.belongs_to_collection){
                const collectionInfo = await axios.get(`${tmdb_main_url}/collection/${media.en.belongs_to_collection.id}`, {params: {...params, language: "en"}});
                media.en.collection = collectionInfo.data;
            }
            console.log(media)
            return media;
        }catch(error){
    ...
    const getEpisodes = async (seriesId, seasonNumber) => {
        const params = {api_key};
        try{
            const episodesInfo = await axios.get(`${tmdb_main_url}/tv/${seriesId}/season/${seasonNumber}`, {params:{...params, language: websiteLang,}});
            return episodesInfo.data?.episodes;
        }catch(error){
            console.error(error);
        }
    }
    ...
    return (
        <Context.Provider value={{
            ...
            languagesOptions,
            fromValue, toValue,
            getMedia, getPerson, getEpisodes,
            voteColor,
            menuVoices
        }}>
            {children}
