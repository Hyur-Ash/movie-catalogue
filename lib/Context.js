...
export const ContextProvider = ({children}) => {
    ...
    const getTmdbConfig = async () => {
        ...
        try{
            ...
            const test = await axios.get(`${tmdb_main_url}/company/33`, {params:{...params}});
    ...
    const [favorites, changeFavorites] = useState({movie: [], tv: [], person:[]});
    const [trashed, setTrashed] = useState({movie: [], tv: [], person:[]});
    useEffect(()=>{
        changeFavorites(currentUser?.favorites ?? {movie: [], tv: [], person:[]});
        setTrashed(currentUser?.trashed ?? {movie: [], tv: [], person:[]});
    },[currentUser]);
