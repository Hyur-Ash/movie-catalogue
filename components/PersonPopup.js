...
export const PersonPopup = ({id}) => {
    ...
    const {
        ...
        scrollId, setScrollId, getPerson
    } = useContext(Context);
    ...
    useEffect(()=>{
        setSelectedRoles(availableRoles[0] ? [availableRoles[0]] : []);
    },[availableRoles]);
    ...
    return isMounted && id && (<>
        {person && person[websiteLang] &&
            ...
            {/* <img alt={person[websiteLang].title} src={person[websiteLang].backdrop_path? `${tmdb_main_url_img_high}/${person[websiteLang].backdrop_path}` : `img/not-found.jpg`}/> */}
            </div>
        }
